const mappers = {
    ...
    $attributes: function (el, mode, v, p, options = {}) {
        ...
        const { style, isComputed, get = 'getAttribute', set = 'setAttribute', remove = 'removeAttribute', defaultAttribute = 'name', toggle = 'toggleAttribute', filter } = options,
            target = style ? (isComputed ? window.getComputedStyle(el) : el.style) : el,
            writable = style ? mode === 'set' && !isComputed : mode === 'set',
            vIsObject = typeof v === 'object'
        p &&= this.toKebabCase(p)
        if (writable) {
            if (p) target[v == null ? remove : !style && typeof v === 'boolean' ? toggle : set](p, v)
            else if (vIsObject) for (const k in v) target[v[k] == null ? remove : !style && typeof v[k] === 'boolean' ? toggle : set](this.toKebabCase(k), v[k])
            if (vIsObject || style) return el
            target[typeof v === 'boolean' ? toggle : set](p || defaultAttribute, v)
            return el
        }
        if (mode !== 'get') return el
        if (p) return target[get](p)
        const r = {},
            iterator = style ? target : el.attributes
        if (iterator.length)
            for (let i = 0, k, l = iterator.length; i < l; i++) {
                k = iterator[i]
    ...
    $: '$data',
    $aria: function (el, mode, v, p, options = {}) {
        return mappers.$data.call(this, el, mode, v, p, { defaultAttribute: 'aria-label', filter: 'aria-' })
    },
    '*': '$aria',
    $style: function (el, mode, v, p, options = {}) {
        return mappers.$attributes.call(this, el, mode, v, p, { style: true, isComputed: false, get: 'getProperty', set: 'setProperty', remove: 'removeProperty' })
    },
    '%': '$style',
    $computed: function (el, mode, v, p, options = {}) {
        return mode === 'get' ? mappers.$attributes.call(this, el, mode, v, p, { style: true, isComputed: true, get: 'getProperty' }) : undefined
    },
    '&': '$computed',
    $content: function (el, mode, v, p, options = {}) {
        if (mode === 'set') el[this.sys.regexp.isHTML.test(v) ? 'innerHTML' : 'textContent'] = v
        return mode === 'set' ? el : this.sys.regexp.isHTML.test(el.textContent) ? el.innerHTML : el.textContent
    },
    '.': '$content',
    $textContent: function (el, mode, v, p, options = {}) {
        if (mode === 'set') el.textContent = v
        return mode === 'set' ? el : el.textContent
    },
    '..': '$textContent',
    $text: function (el, mode, v, p, options = {}) {
        if (mode === 'set') el.innerText = v
        return mode === 'set' ? el : el.innerText
    },
    '...': '$text',
    $html: function (el, mode, v, p, options = {}) {
        ...
        if (Array.isArray(v)) {
            for (const item of v) {
                const itemNode = item?.$tag ? document.createElement(item.$tag) : p ? document.createElement(p) : new DocumentFragment(),
                    itemPromises = []
            ...
            for (const k in v) promises.push(this.render(el, v[k]))
        } else {
            el.innerHTML = `${v}`
        }
    ...
    $tag: function (el, mode, v, p, options = {}) {
        if (!(el && el instanceof HTMLElement)) return
        if (mode === 'set') v == null ? el.removeAttribute('is') : el.setAttribute('is', v.toLowerCase())
        return mode === 'set' ? el : (value.getAttribute('is') || value.tagName).toLowerCase()
    },
    $parent: function (el, mode, v, p, options = {}) {
        if (!(el && el instanceof HTMLElement)) return
        el = this.app._components.nativesFromVirtuals.get(el) ?? el
        return mode === 'set' ? el : this.flatten(p ? el.closest(p) : el.parentElement)
    },
    '^': '$parent',
    $position: function (el, mode, v, p, options = {}) {
        ...
        if (!inserters.has(inserter)) return el
        const fragment = new DocumentFragment(),
            promises = []
        if (Array.isArray(v)) {
            for (const item of v) {
                const itemNode = item?.$tag ? document.createElement(item.$tag) : new DocumentFragment(),
                    itemPromises = []
                if (item?.$tag) delete item.$tag
                itemPromises.push(this.render(itemNode, item))
        ...
        const returnValue = inserter === 'replaceWith' ? Array.from(fragment.children) : el
        return Promise.all(promises)
            .then(() => el[inserter](fragment))
            .then(() => (returnValue.length === 1 ? returnValue[0] : returnValue))
    },
    $event: function (el, mode, v, p, options = {}) {
        return mode === 'set' ? el : p ? this.flatten(options?.detail?.[p]) : this.flatten(options)
    },
    '!': '$event',
            ...
            default:
                ...
                switch (type) {
                    ...
                    case 'radio':
                        ...
                        return r
                    default:
                        if (mode === 'set') el.value = v
                        return mode === 'set' ? el : el.value
                }
        }
    },
    '[]': '$form',
    ...
    $options: function (el, mode, v, p, options = {}) {
        ...
        if (mode === 'set') {
            const optionElements = [],
                isOptGroup = el instanceof HTMLOptGroupElement
            if (v && typeof v === 'object') {
                const vIsArray = Array.isArray(v),
                    optionsMap = vIsArray ? {} : v
            ...
            el.replaceChildren(...optionElements)
            return el
        }
        const rObj = {},
            rArr = []
        ...
        for (const optionElement of el.children) {
            ...
            const optionText = optionElement.textContent.trim(),
                optionValue = optionElement.getAttribute('value') || optionText
...
export default {
    processElementMapper: async function (element, mode, prop, value, isVoidElement) {
        ...
        if (prop in mappers)
            return mode === 'has' || (typeof mappers[prop] === 'string' ? mappers[mappers[prop]] : mappers[prop]).call(this, element, mode, value, prop, { isVoidElement })
        const propFlag = prop[0],
            propMain = prop.slice(1)
        if (propFlag in mappers)
            return (typeof mappers[propFlag] === 'string' ? mappers[mappers[propFlag]] : mappers[propFlag]).call(this, element, mode, value, propMain, { isVoidElement })
        ...
        if (mode === 'set') element[prop] = value
        return mode === 'has' ? prop in element : mode === 'set' ? element : this.flatten(element[prop])
    },
}
